...
Seeds the root categories and the settings row for a user.
"""
...
class InitialiseUserWorkspaceUseCase:
    """Seeds a user's root categories and settings."""
    ...
    def execute(self) -> None:
        """Ensure the user has their root categories and settings.
        ...
        """
        try:
            self._ensure_default_root_categories()
    ...
    def _ensure_default_root_categories(self) -> None:
        """Create any missing root category for the user.

        Only the roots are seeded: everything below them is the user's own, and
        a payment can be attributed to a root directly, so there is nothing
        hidden to stand in for one.
        """
        existing_names = {
            category.name
    ...
    def _ensure_default_settings(self) -> None:
        ...
        Create-if-missing like the categories above: a row already carrying a
