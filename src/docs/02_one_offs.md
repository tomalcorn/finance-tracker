...
- If you come to buy the item, move money from the easy savings into your current account. You don't need to log buying the item with a category, only log the incremental payments into your easy-access savings as `Investments`.
...
Add that amount as a new One-Offs entry and then next month, assign as much of its total as you can to `Planned Spend` leave it sitting there. You don't need to bank it or do anything with it, just make sure your One-Offs `Planned Spend` total doesnt go over 100%. Reduce the total by the amount you had in `Planned Spend` at the end of that month, then continue until you've "paid" it all off.
