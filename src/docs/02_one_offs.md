...
- Update `Planned` when you want to earmark money for that goal this month.
...
`Spent/Banked` is that goal's `Starting` plus every payment attributed to it,
so it rises by what you just banked. `Planned` stays where you set it:
...
- Use `Planned` for the money you are assigning right now.
